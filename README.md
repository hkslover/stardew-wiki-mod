...
| **存档信息** | 按需读取时间、天气、手持物、筛选背包、状态、技能、关系和任务日志 |
...
手持物来自提交问题时选中的快捷栏格子；背包默认只返回容量或本地筛选命中的物品，不会自动把完整背包注入问题。
...
- `AllowFullInventoryRead`：是否允许玩家明确要求时使用完整背包读取，默认为 `true`；关闭后仍可查询容量和筛选物品。
